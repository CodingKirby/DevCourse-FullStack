...
INSERT INTO books (title, form, isbn, summary, detail, author, pages, contents, price, pub_date)
VALUES ("흥부와 놀부들", "종이책", 3, "제비..", "까만 제비..", "김제비", 100, "목차입니다.", 20000, "2024-10-08");
...
INSERT INTO books (title, img, category_id, form, isbn, summary, detail, author, pages, contents, price, pub_date)
VALUES ("효녀 심청", 12, 0, "종이책", 9, "심청아..", "공양미 삼백석..", "김심청", 100, "목차입니다.", 20000, "2024-10-03");

INSERT INTO books (title, img, category_id, form, isbn, summary, detail, author, pages, contents, price, pub_date)
VALUES ("혹부리 영감", 22, 2, "ebook", 10, "노래 주머니..", "혹 두개 되버림..", "김영감", 100, "목차입니다.", 20000, "2024-10-05");
...
// 주문 상세 목록 입력
INSERT INTO ordered (order_id, book_id, quantity)
VALUES (order_id, 1, 1);
INSERT INTO ordered (order_id, book_id, quantity)
VALUES (order_id, 3, 2);


SELECT max(id) FROM Bookshop.ordered;
...
// 좋아요 개수 세기
SELECT *, (
	SELECT count(*) FROM likes WHERE book_id = books.id
) AS likes FROM books;

// 좋아요 표시 여부
SELECT EXISTS (
	SELECT * FROM likes WHERE user_id = 1 AND book_id = 1
);
...
SELECT *,
	...
	(SELECT EXISTS (
	SELECT * FROM likes WHERE user_id = 1 AND book_id = 1)) AS liked
